...
class OrderService{
    ...
    public function takeOrder(Order $order, User $user): Order{
        if (! $user->isDriver()) {
            throw ValidationException::withMessages([
                'user' => ['Faqat driver orderni olishi mumkin'],
            ]);
        }
        if ($order->driver_id !== null) {
            throw ValidationException::withMessages([
                'order' => ['Bu order allaqachon boshqa driver tomonidan olingan'],
            ]);
        }
        if ($order->status !== Order::STATUS_PACKING) {
            throw ValidationException::withMessages([
                'status' => ['Faqat packing holatidagi orderni olish mumkin'],
            ]);
        }
        $order->update([
            'driver_id' => $user->id,
    ...
    public function deliverOrder(Order $order, User $user): Order{
        if (! $user->isDriver()) {
            throw ValidationException::withMessages([
                'status' => ['Siz driver emassiz'],
            ]);
        }
        if ($order->driver_id !== null) {
            throw ValidationException::withMessages([
                'status' => ['Bu sizning order emas!'],
            ]);
        }
        if ($order->status !== Order::STATUS_ON_WAY) {
            throw ValidationException::withMessages([
                'status' => ['Avval Order oling!'],
            ]);
        }
        $order->update([
    ...
    public function packingDeliveries(int $perPage = 10){
